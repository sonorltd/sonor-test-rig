...
`install.sh` header for why). In the Imager settings: hostname `sonorpi`, user `sonorpi`, SSH on, Wi-Fi if
no Ethernet, UK locale/timezone.

```bash
ssh sonorpi@sonorpi.local
...
```bash
scp -r "$HOME/Code/Sonor/APP - C-Bus" sonorpi@sonorpi.local:~/sonor/cbus
ssh sonorpi@sonorpi.local sonor-rig install cbus
```
